...
async function to_segments(grid) {
  ...
  await m_js_for_each(grid, async (row, row_index) => {
    ...
    await m_js_for_each(row, async (col, col_index) => {
      ...
      let neighbors = {
        up: {row:row_index-1,col:col_index},
        down: {row:row_index+1,col:col_index},
        left: {row:row_index,col:col_index-1},
        right: {row:row_index,col:col_index+1},
      };
      for (let neighbor in neighbors) {
        if (neighbors[neighbor] === 0) {
          await list_add(result, )
        }
      }
      console.log({
        row_index,
